...
def _default_home_dir() -> Path:
    ...
    for candidate in candidates:
        ...
        try:
            ...
            continue
        # 2026-04-21 review comment #8 + PR-#13: mirror the
        # passport.py guard. Chmod is best-effort on filesystems that
        # don't support it; emit a stderr warning so operators see
        # the security trade-off (private-key material may be
        # world-readable under the caller's umask). Set VIBAP_HOME
        # to a chmod-capable fs to silence.
        try:
...
def verify_pop(
    ...
    try:
        kb_claims = jwt.decode(
            ...
            options={"verify_aud": False, "require": ["iat", "nonce", "sd_hash"]},
        )
    ...
    if kb_claims.get("sd_hash") != expected_hash:
        raise PermissionError(
...
def verify_passport(
    ...
    if parent_jti is not None:
        ...
        if parent_token is not None:
            ...
            # K1 fix: verify chain[0].token_hash matches the supplied parent
            # token. This ensures the first chain link genuinely represents the
            # parent — not a spliced token from a different derivation path.
            # Deeper hops are trusted transitively because the chain is signed
            # by the same key; in a multi-key deployment (AAT I1), each hop's
            # token_hash would need independent verification.
            chain = delegation_chain_entries(claims)
            if chain:
                chain_token_hash = chain[0].get("token_hash")
                if chain_token_hash is not None:
                    if chain_token_hash != _token_sha256(parent_token):
                        raise PermissionError(
                            "delegation chain splice detected: "
                            "chain[0].token_hash does not match parent token"
                        )
    else:
        delegation_chain_entries(claims)
...
def derive_child_passport(
    ...
    """
    parent = verify_passport(parent_token, public_key)
    ...
    # K1 fix: embed parent's own token hash in the chain link so downstream
    # verifiers can confirm chain[0] IS the declared parent without holding
    # the raw parent token. This closes the multi-hop splice gap.

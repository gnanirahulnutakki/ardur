...
A AutoGen agent making tool calls through Ardur's governance proxy. The agent receives an Ardur-issued mission credential, calls a small set of tools (read, write, summarize), and Ardur:
...
Adapter code lives in the private research repo at `vibap-prototype/demos/AutoGen/`. The lift requires the same Wolverine/Radiantic → Ardur rename pass as the rest of the runtime (clean break, no backward-compat dual-type — see `docs/specs/README.md` "Protocol identifier rename" section).

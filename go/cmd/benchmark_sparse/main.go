...
func main() {
	...
	outDir := flag.String("out", filepath.Join("benchmark", "results", "pack-v0.1-sparse"), "directory for generated report artifacts")
	flag.Parse()

	report, err := benchmark.RunPackUncheckedEvents(*packDir)
	if err != nil {
		log.Fatalf("run sparse benchmark pack: %v", err)
	}
	if err := benchmark.WriteReport(report, *outDir); err != nil {
		log.Fatalf("write sparse benchmark report: %v", err)
	}
